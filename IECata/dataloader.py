...
class DTIDataset(data.Dataset):
    def __init__(self, list_IDs, df, cfg, max_drug_nodes=290):
    ...
    def __getitem__(self, index):
        ...
        if self.cfg['protein_encode']['input'] == 'prottrans':
            v_p = embed_dataset(v_p)
            pad_width = ((0, 1200-len(v_p)), (0, 0))  # 在行方向上填充876行，在列方向上不填充
            constant_values = 0  # 填充值为0
            # 使用np.pad()函数填充矩阵
            v_p = np.pad(v_p, pad_width, mode='constant', constant_values=constant_values)
